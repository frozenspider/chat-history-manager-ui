...
trait MessagesRenderingComponent[MD] {
  ...
  def render(dao: ChatHistoryDao, cc: CombinedChat, msgs: Seq[Message], beginReached: Boolean, showTop: Boolean): MD
  ...
  def prepend(dao: ChatHistoryDao, cc: CombinedChat, msgs: Seq[Message], beginReached: Boolean): MD

  /** Append current content given messages. Removes "loading" section, if any. */
  def append(dao: ChatHistoryDao, cc: CombinedChat, msgs: Seq[Message], endReached: Boolean): MD
