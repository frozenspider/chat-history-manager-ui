...
/** Copy-paste of {@link javax.swing.text.html.Map} with {@code javax.swing.text.html} classes replaced where appropriate */
...
class CustomMap implements Serializable {
    ...
    private Vector<CustomMap.RegionContainment>           areas;
    ...
    public AttributeSet getArea(int x, int y, int width, int height) {
        ...
        if (numAttributes > 0) {
            ...
            if (areas == null) {
                areas = new Vector<CustomMap.RegionContainment>(numAttributes);
            }
            for (int counter = 0; counter < numAttributes; counter++) {
                ...
                CustomMap.RegionContainment rc = areas.elementAt(counter);
     ...
     */
    protected CustomMap.RegionContainment createRegionContainment
        ...
        if (shape instanceof String) {
            ...
            CustomMap.RegionContainment rc = null;

            try {
                if (shapeString.equals("rect")) {
                    rc = new CustomMap.RectangleRegionContainment(attributes);
                }
                else if (shapeString.equals("circle")) {
                    rc = new CustomMap.CircleRegionContainment(attributes);
                }
                else if (shapeString.equals("poly")) {
                    rc = new CustomMap.PolygonRegionContainment(attributes);
                }
                else if (shapeString.equals("default")) {
                    rc = CustomMap.DefaultRegionContainment.sharedInstance();
                }
     ...
     */
    static class RectangleRegionContainment implements CustomMap.RegionContainment {
        ...
        public RectangleRegionContainment(AttributeSet as) {
            int[]    coords = CustomMap.extractCoords(as.getAttribute(HTML.Attribute.COORDS));
     ...
     */
    static class CircleRegionContainment implements CustomMap.RegionContainment {
     ...
     */
    static class DefaultRegionContainment implements CustomMap.RegionContainment {
        /** A global shared instance. */
        static CustomMap.DefaultRegionContainment si = null;

        public static CustomMap.DefaultRegionContainment sharedInstance() {
            if (si == null) {
                si = new CustomMap.DefaultRegionContainment();
            }
            return si;
        }
